...
import TeamNameAndLeaderStep from "./steps/TeamNameAndLeaderStep"
import SingleMemberStep from "./steps/SingleMemberStep"
import TeamQuestionsStep from "./steps/TeamQuestionsStep"
import ResponsibilityEnforcement from "../components/ResponsibilityEnforcement"
import { validateTeamName, validateMember, isResponsibilityCoverageValid, validateMobaiForm } from "../lib/validation"
import { submitMobaiRegistration } from "../lib/api"
...
function scrollToFirstError(errors, step) {
    const run = () => {
        let id = null
        if (step === 0) {
            if (errors?.team_name) id = "mobai-team_name"
            else if (errors?.members?.[0]) id = `mobai-member-0-${Object.keys(errors.members[0])[0]}`
        } else if (step >= 1 && step <= 5 && errors?.members?.[step]) {
            id = `mobai-member-${step}-${Object.keys(errors.members[step])[0]}`
        } else if (step === 6) {
            if (errors?.worked_together_before) id = "mobai-worked_together_before"
            else if (errors?.team_quality_opinion) id = "mobai-team_quality_opinion"
            else if (errors?.prize_not_wanted) id = "mobai-prize_not_wanted"
        }
        if (id) {
...
const RegistrationForm = () => {
    ...
    const validateStep = (step) => {
        ...
        if (step === 0) {
            ...
            if (teamNameErr) newErrors.team_name = teamNameErr
            const leaderErr = validateMember(members[0], 0)
            if (Object.keys(leaderErr).length > 0) newErrors.members = [leaderErr, {}, {}, {}, {}, {}]
        }
        ...
        if (step === 6) {
            if (!teamQuestions.worked_together_before) newErrors.worked_together_before = "Required"
            const quality = (teamQuestions.team_quality_opinion ?? "").trim()
            if (quality.length < 30) newErrors.team_quality_opinion = "Min 30 characters"
            const prize = (teamQuestions.prize_not_wanted ?? "").trim()
            if (prize.length < 10) newErrors.prize_not_wanted = "Min 10 characters"
        }
    ...
    const renderStep = () => {
        switch (currentStep) {
            ...
            case 6:
                return (
                    <TeamQuestionsStep
                        formData={teamQuestions}
                        setFormData={setTeamQuestions}
                        errors={errors}
                    />
                )
            default:
