...
const AgendaItem = ({ item, index }) => {
    ...
    return (<>
            ...
            <div className={cn("hidden md:flex relative items-center gap-8", isLeft ? "justify-start" : "justify-end")}>
                {isLeft ? (<>
                        {/* Left side: Card */}
...
const EventAgenda = () => {
    return (<section className="relative w-full overflow-hidden py-20" id="agenda">
            {/* Title */}
            ...
            <div className="hidden md:block relative max-w-[80%] lg:max-w-[70%] mx-auto mb-12">
                ...
                <div className="space-y-16">
                    {agendaItems.map((item, index) => (<AgendaItem key={index} item={item} index={index}/>))}
                </div>
            </div>
        ...
        }} className="absolute cursor-pointer left-[calc(50%_-_46px)] bottom-[26px] w-[94px] h-[30px] hidden md:block" alt="Line" src="/images/down_arrow.svg"/>

            {/* Corner Decorative Groups */}
            <img className="absolute top-2 right-2 w-20 h-auto -rotate-90
            sm:top-3 sm:right-4 sm:w-24
            md:top-4 md:right-6 md:w-32
            lg:top-5 lg:right-5 lg:w-40" alt="Group" src="/images/corners.svg"/>
            <img className="absolute bottom-2 left-2 w-20 h-auto rotate-90
            sm:bottom-3 sm:left-4 sm:w-24
            md:bottom-4 md:left-6 md:w-32
            lg:bottom-5 lg:left-5 lg:w-40" alt="Group" src="/images/corners.svg"/>

            <img src="/images/shadow.svg" alt="Shadow" className="
                        absolute
                        -top-40 -left-40
