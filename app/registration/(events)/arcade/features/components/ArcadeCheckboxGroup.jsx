...
const ArcadeCheckboxGroup = ({ label, options, selected, onChange, error }) => {
    ...
    return (<div className="flex flex-col gap-3 w-full">
      ...
      <div className="relative z-10 flex flex-wrap items-center justify-start gap-4 sm:gap-6" style={{
            width: "100%",
            minHeight: "61px",
        ...
        {options.map((option) => {
            ...
            return (<label key={option.value} className="flex items-center gap-2 cursor-pointer font-futura text-white" style={{ fontSize: "16px" }}>
              <input type="checkbox" value={option.value} checked={isSelected} onChange={() => toggleSelection(option.value)} className="sr-only"/>
              <span aria-hidden="true" className="flex-shrink-0 flex items-center justify-center transition-colors duration-200" style={{
                    width: "16px",
                    height: "16px",
                    borderRadius: "2px",
