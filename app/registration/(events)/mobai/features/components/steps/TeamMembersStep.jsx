...
const TeamMembersStep = ({ members, setMembers, errors }) => {
    ...
    return (<div className="space-y-8">
            ...
            {[1, 2, 3, 4, 5].map((i, idx) => (<CyberCard key={i}>
                    <MemberCard memberIndex={i} member={members[i] ?? {}} errors={errors?.members?.[i]} onChange={handleMemberChange} isLeader={false} icon={MEMBER_ICONS[idx]}/>
                </CyberCard>))}
        </div>);
};
