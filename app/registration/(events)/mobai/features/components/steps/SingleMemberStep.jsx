...
const SingleMemberStep = ({ memberIndex, members, setMembers, errors }) => {
    ...
    return (<MemberCard memberIndex={memberIndex} member={members[memberIndex] ?? {}} errors={errors?.members?.[memberIndex]} onChange={handleMemberChange} isLeader={false} icon={icon} fieldIdPrefix={`mobai-member-${memberIndex}`}/>);
};
