...
const CyberCard = ({ children, className = "", shadow = true, mentors = "" }) => {
    ...
    const [polygonShape, setPolygonShape] = useState(() => typeof window === "undefined" ? getPolygonShape() : getPolygonShape(window.innerWidth));
    useEffect(() => {
        const handleResize = () => setPolygonShape(getPolygonShape(window.innerWidth));
        window.addEventListener("resize", handleResize);
        return () => window.removeEventListener("resize", handleResize);
    }, []);
    return (<div className={`relative ${className} h-full`}>
            {/* Lens flare - Top Right Corner */}
            <div className="absolute top-25 -right-4 pointer-events-none z-30" style={{
            width: "35px",
            height: "100px",
            ...
            <div className="absolute -bottom-4 left-20 pointer-events-none z-30" style={{
            width: "100px",
            height: "40px",
            background: "radial-gradient(ellipse at 50% 50%, rgba(193, 177, 193, 1) 0%, rgba(193, 177, 193, 0.9) 15%, rgba(193, 177, 193, 0.3) 35%, transparent 65%)",
            filter: "blur(2px)",
            ...
            filter: shadow ? "drop-shadow(0 0 20px rgba(88, 34, 94, 1)) drop-shadow(0 15px 50px rgba(0, 0, 0, 0.9))" : "none"
                ...
                <div className="relative bg-[#966996] p-[5px]" style={{
            ...
            filter: "drop-shadow(0 0px 5px rgba(179, 202, 202, 0.8)) drop-shadow(0 0 15px rgba(0, 0, 0, 0.7))"
                        ...
                        <div className="relative" style={{
            clipPath: polygonShape,
            backgroundImage: `url(/images/mobai/cardbg.webp)`,
