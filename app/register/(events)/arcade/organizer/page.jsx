...
import ArcadeCard from "../features/components/ArcadeCard";
import ArcadeCheckboxGroup from "../features/components/ArcadeCheckboxGroup";
import ArcadeInput from "../features/components/ArcadeInput";
import ArcadeRadioToggle from "../features/components/ArcadeRadioToggle";
...
const validateForm = (data) => {
    ...
    if (data.isMember === null)
        errors.isMember = "Please select Yes or No.";
...
const OrganizersRegPage = () => {
    ...
    return (<div className="arcade-registration-page arcade-organizers-reg-page overflow-hidden" style={{ background: "#080000", minHeight: "100vh" }}>
      <style>{`
      ...
      <div className={`arcade-registration-content relative z-10 ${submitSuccess ? "min-h-[100dvh] flex flex-col justify-center py-0" : "py-8 sm:py-12 pt-20"}`}>
        {/* Title */}
        {!submitSuccess && (<div className="text-center mb-24 sm:mb-32">
                ...
                <div className="org-card-body relative flex h-full flex-col">
                  <div className="flex flex-1 items-start pt-5 sm:pt-4">
                    <div className="relative z-10 grid w-full content-start auto-rows-min grid-cols-1 md:grid-cols-2 gap-x-8 lg:gap-x-[130px] gap-y-10 sm:gap-y-10 pb-3 sm:pb-4">
                      <ArcadeInput label="Your Name" placeholder="Enter your full Name" value={formData.name} onChange={(e) => updateField("name", e.target.value)} error={errors.name}/>
                      <ArcadeInput label="Your school" placeholder="Enter your school name" value={formData.school} onChange={(e) => updateField("school", e.target.value)} error={errors.school}/>
                      <ArcadeInput label="Your email" placeholder="example@gmail.com" value={formData.email} onChange={(e) => updateField("email", e.target.value)} error={errors.email}/>
                      <ArcadeInput label="Your phone number" placeholder="Enter your phone Number" value={formData.phone} onChange={(e) => updateField("phone", e.target.value)} error={errors.phone}/>
                      <ArcadeYearSelect className="org-personal-last-field" label="Year of studying" value={formData.schoolYear} onValueChange={(value) => updateField("schoolYear", value)} error={errors.schoolYear}/>
                    </div>
                  </div>
                </div>
              </ArcadeCard>
        ...
        cardHeight="auto" contentPadding="px-8 sm:px-14 lg:px-16 py-10 sm:py-10" topRightCorner="/images/arcade/top_right_org.png" bottomLeftCorner="/images/arcade/bottom_left_org.png">
                <div className="org-card-body relative flex h-full flex-col z-10 pt-5 sm:pt-4">
                  <div className="org-questions-stack flex flex-col gap-8 sm:gap-10">
                    <div className="org-row-member flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      <label className="font-futura text-white font-medium" style={{
                ...
                lineHeight: "1.3",
                      ...
                      <div className="org-control-206 org-mobile-yesno md:ml-auto">
                        <ArcadeRadioToggle name="isMember" options={yesNoOptions} value={formData.isMember} onChange={(val) => updateField("isMember", val)} error={errors.isMember}/>
                      </div>
                    </div>
                ...
                lineHeight: "1.3",
                      ...
                      <div className="org-control-311 org-checkbox org-mobile-days md:ml-auto">
                        <ArcadeCheckboxGroup options={daysOptions} selected={formData.availableDays} onChange={(val) => updateField("availableDays", val)} error={errors.availableDays}/>
                      </div>
                    </div>
                ...
                lineHeight: "1.3",
                      ...
                      <div className="org-control-206 org-mobile-yesno md:ml-auto">
                        <ArcadeRadioToggle name="availablePreMeet" options={yesNoOptions} value={formData.availablePreMeet} onChange={(val) => updateField("availablePreMeet", val)} error={errors.availablePreMeet}/>
                      </div>
                    </div>
                ...
                lineHeight: "1.3",
                      ...
                      <div className="org-control-206 org-mobile-yesno md:ml-auto">
                        <ArcadeRadioToggle name="hasExperience" options={yesNoOptions} value={formData.hasExperience} onChange={(val) => updateField("hasExperience", val)} error={errors.hasExperience}/>
                      </div>
                    </div>

                    <div className="org-mobile-experience w-full mt-2">
                      <ArcadeTextarea label="Describe your experience :" placeholder="Write here .." className="no-scrollbar" value={formData.experienceDescription} onChange={(e) => updateField("experienceDescription", e.target.value)} error={errors.experienceDescription} showErrorState={false} textareaStyle={{
                minHeight: "120px",
                ...
                lineHeight: "1.3",
                      ...
                      <div className="org-control-206 org-mobile-role md:ml-auto">
                        <ArcadeRadioToggle name="role" options={roleOptions} value={formData.role} onChange={(val) => updateField("role", val)} error={errors.role}/>
                      </div>
                    </div>

                    <div className="w-full mt-2 flex flex-col gap-3">
                      <label className="font-futura text-white font-medium mb-1" style={{
                fontSize: "clamp(15px, 1.5vw, 20px)",
                lineHeight: "1.3",
            }}>
                      ...
                      <div className="org-control-full org-checkbox org-mobile-shifts">
                        <ArcadeCheckboxGroup options={shiftOptions} selected={formData.preferredShifts} onChange={(val) => updateField("preferredShifts", val)} error={errors.preferredShifts}/>
                      </div>
                    </div>
                  </div>
                ...
                borderRadius: "40px",
                    ...
                    <div className="order-1 sm:order-2" style={{
                filter: "drop-shadow(0px 0px 4px rgba(255, 255, 255, 0.25))",
                borderRadius: "40px",
            }}>
