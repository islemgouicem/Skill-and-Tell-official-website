      ...
      <ArcadeCard className="arcade-mobile-no-button-card" size="sm" title="Team Information" icon={<MemberIcon />} cardHeight="clamp(226px, 52vw, 254px)">
        <div className="flex h-full flex-col justify-center items-center sm:items-start sm:-mt-1">
          <div className="w-full" style={{ maxWidth: "clamp(320px, 52vw, 467px)" }}>
            <ArcadeInput label="Team Name" placeholder="Enter The Team Name" value={formData.teamName} onChange={(e) => updateField("teamName", e.target.value)} error={errors.teamName}/>
          </div>
        </div>
      </ArcadeCard>
            ...
            <div className="flex flex-1 items-start pt-5 sm:pt-4">
              <div className="relative grid w-full content-start auto-rows-min grid-cols-1 md:grid-cols-2 gap-x-8 lg:gap-x-[130px] gap-y-10 sm:gap-y-10 pb-3 sm:pb-4">
                <div className="relative z-10">
                  <ArcadeInput label="Leader Name" placeholder="Enter your full Name" value={formData.leaderName} onChange={(e) => updateField("leaderName", e.target.value)} error={errors.leaderName}/>
                </div>
                <div className="relative z-10">
                  <ArcadeInput label="Leader Email" placeholder="Enter Your Email" value={formData.leaderEmail} onChange={(e) => updateField("leaderEmail", e.target.value)} error={errors.leaderEmail}/>
                </div>
                <div className="relative z-10">
                  <ArcadeInput label="Leader Number" placeholder="Enter your phone Number" value={formData.leaderNumber} onChange={(e) => updateField("leaderNumber", e.target.value)} error={errors.leaderNumber}/>
                </div>
                <div className="relative z-10">
                  <ArcadeYearSelect label="Year of studying" value={formData.leaderYear} onValueChange={(value) => updateField("leaderYear", value)} error={errors.leaderYear}/>
                </div>
                <div className="relative z-10 mt-2 mb-4">
                  <ArcadeYearSelect label={<>
                        How many times did you previously participate in Arcade
                        {" "}
                        <span style={{ fontFamily: "Arial, system-ui, sans-serif" }}>?</span>
                      </>} placeholder="Select number" value={formData.leaderParticipation} onValueChange={(value) => updateField("leaderParticipation", value)} error={errors.leaderParticipation} options={participationOptions}/>
                </div>
              </div>
            </div>
