...
function Navbar() {
    ...
    return (<>
            ...
            <header className={`fixed w-full z-50 transition-all duration-500 ease-in-out backdrop-blur-sm shadow-lg ${isVisible ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"} bg-purple-2`} onMouseEnter={showNavbar} onMouseLeave={() => !isOpen && startHideTimer(2000)}>
                <div className="container mx-auto flex h-20 items-center justify-between px-4 md:px-6">
                    <a href="#hero" aria-label="logo" className="flex items-center gap-2" onClick={(e) => handleNavLinkClick(e, "#hero")}>
                        <Image src={"/images/eunoia.svg"} width={100} height={100} alt="eunoia Logo" className="transition-transform duration-300" fetchPriority="low"/>
                    </a>

                    {/* Desktop Navigation */}
                    <nav className="hidden lg:flex items-center gap-6">
                        {navLinks.map(({ name, href }) => (<a key={name} href={href} className="text-white/75 text-lg font-medium hover:text-gold transition-colors relative group px-2 py-1 rounded-md" onClick={(e) => handleNavLinkClick(e, href)}>
                                {name}
