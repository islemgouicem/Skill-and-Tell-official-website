...
export default function Error({ error, reset, }) {
    return (<main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#23003B] px-6">
      ...
      <div className="relative z-10 w-full max-w-2xl rounded-3xl border border-violet-500/40 bg-[#2B0546]/80 p-12 backdrop-blur-xl shadow-[0_0_60px_rgba(124,58,237,.25)]">
        ...
        {process.env.NODE_ENV === "development" && (<div className="mt-8 rounded-xl border border-red-500/20 bg-red-500/10 p-4">
        ...
        <div className="mt-10 flex flex-col gap-4 sm:flex-row sm:justify-center">
          ...
          <button onClick={() => reset()} className="flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-500 to-orange-600 px-6 py-3 font-semibold text-white transition hover:scale-105">
            <RotateCcw size={18}/>
            Try Again
          </button>

          <Link href="/" className="flex items-center justify-center gap-2 rounded-xl border border-violet-400 px-6 py-3 font-semibold text-white transition hover:bg-violet-500/10">
            <House size={18}/>
            Back Home
          </Link>
