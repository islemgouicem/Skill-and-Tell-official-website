...
function MobileNavbar({ isOpen, setIsOpen, navLinks, handleNavLinkClick }) {
    return (<Sheet open={isOpen} onOpenChange={setIsOpen}>
            ...
            <SheetContent side="right" className="border-Main-400 w-[250px] text-space-text mob-navbar-bg">
                <a href="#hero" aria-label="logo" className="flex items-center gap-2 mb-2" onClick={(e) => handleNavLinkClick(e, "#hero")}>
                    <Logo size={100}/>
                </a>
                <nav className="grid gap-4 py-6">
                    {navLinks.map(({ name, href }) => (<a key={name} href={href} className="flex w-full items-center py-2 text-lg font-semibold hover:text-[#d7aeff] transition-colors" onClick={(e) => handleNavLinkClick(e, href)}>
                            {name}
                        </a>))}
                </nav>
            </SheetContent>
