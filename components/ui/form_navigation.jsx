...
const FormNavigation = () => {
    ...
    return (<div className="flex justify-between items-center mt-6 pt-6 border-t border-space-subtle">
            {/* Back Button */}
            ...
            : prevStep} variant="ghost" className="text-white/80 border border-Main-500 rounded-sm hover:bg-space-light disabled:opacity-50">
                <ArrowLeft className="w-5 h-5 mr-2"/>
                Back
            </Button>

            {/* Next or Submit Button */}
            {currentStep < totalSteps ? (<Button onClick={nextStep} className="gradient-buttons rounded-sm text-white hover:from-space-orange-light hover:to-space-purple">

                    Next
                    <ArrowRight className="w-5 h-5 ml-2"/>
                </Button>) : (<Button onClick={submitForm} disabled={isSubmitting} className="bg-gradient-to-r rounded-sm from-green-500 to-green-600 text-white hover:from-green-600 hover:to-green-700 px-4 lg:px-8">
                    {isSubmitting ? (<>
